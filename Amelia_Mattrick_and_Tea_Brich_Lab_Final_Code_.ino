...
///Amelia Mattrick and Teá Brich 
///Lab Final 
///NMD 211 Lab 1
///12/10/21
#include <SPI.h>

#include <MD_MAX72xx.h> // We use this to control the 8x8 LED matrix Displays 
#include <NewPing.h> // We use NewPing to control the Ultrasonic Sensors 
...
#define LIGHT A5 // use Analogue pin 5 to read the light value from the LDR

// We are using hardware SPI which automatically 
MD_MAX72XX mx = MD_MAX72XX(HARDWARE_TYPE,CS_PIN, MAX_DEVICES); // Initialise the 2 Matrix displays
...
#define maxDist 1000 // the max distance for the ultrasonic pulse
...
void loop()
{
  ...
  // if distance1 is greater than distance2 and distance1 is also greater than 0, then we want to look right  
  else if ( distance2 < distance1 && distance1 > 0)
  {
    ShowEye_Right();
    currentState = 1;
  }
  // if distance2 is greater than distance1 and distance2 is also greater than 0, then we want to look left  
